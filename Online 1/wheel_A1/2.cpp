...
void keyboardListener(unsigned char key, int x,int y){
	switch(key){
		...
		case 'w':
			...
			robo_pos.x -= 2*cos(rotateAngle*acos(-1.0)/180.0) ;
            robo_pos.y -= 2*sin(rotateAngle*acos(-1.0)/180.0) ;
...
*/
		...
		case 'a':
			rotateAngle += 4 ;
			dir.x += 1*cos(rotateAngle*acos(-1.0)/180.0);
			dir.y += 1*sin(rotateAngle*acos(-1.0)/180.0);
			break;
...
void draw_board(){
	...
	glTranslatef(robo_pos.x , robo_pos.y, robo_pos.z);
...
void init(){
	...
	************************/
	...
	pos = point(0,0,0) ;
	robo_pos = point(0,0,0) ;
	dir = point(30, 0,radius);
