...
public:
    ...
    virtual double intersect(Ray &ray, Color &color, int level) {
        ...
        for(Light light: lights) {
            ...
            Point R = get_reflection_vector(L, N);
            Point eye = Point(100,100,50);//default camera position
            ...
            if (t_tgt > t_other_min) continue;//this ray is blocked by another object
...
public:
    ...
    double get_intersecting_t(Ray &ray) override {
        if(ray.dir[2] == 0) return -1;//if ray parallel to xy plane,doesnt intersect
